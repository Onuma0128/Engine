...
class ObjectShadowMapDepthRootSignature : public RootSignatureBase
{
public:
	...
	ComPtr<ID3D12RootSignature> BuildRootSignature(ID3D12Device* device, PostEffectType effectType = PostEffectType::kNone) override
	{
		...
		materialRange[0].BaseShaderRegister = 1; //3から始まる
		...
		rootParameters[2].DescriptorTable.pDescriptorRanges = materialRange;				//Tableの中身の配列を指定
		rootParameters[2].DescriptorTable.NumDescriptorRanges = _countof(materialRange);	//Tableで利用する数
