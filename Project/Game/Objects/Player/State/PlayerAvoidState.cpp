...
void PlayerAvoidState::Init()
{
	...
	if (velocity_.Length() == 0.0f) {
		...
		player_->SetTransformRotation(
			NumaEngine::Quaternion::DirectionToQuaternion(player_->GetTransform().rotation_, velocity_, 1.0f));
		rotateY_ = NumaEngine::Quaternion::ExtractYawQuaternion(player_->GetTransform().rotation_);
	}
}
...
void PlayerAvoidState::Update()
{
	...
	// 回転を適応
	NumaEngine::Quaternion rotateX = NumaEngine::Quaternion::MakeRotateAxisAngleQuaternion
	(NumaEngine::Vector3::ExprUnitX, (avoidTime_ * 2.0f) * std::numbers::pi_v<float>);
...
const NumaEngine::Vector3 PlayerAvoidState::CreateVelocity()
{
	// Inputを取得
