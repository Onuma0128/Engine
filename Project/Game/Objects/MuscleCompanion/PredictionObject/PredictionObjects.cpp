...
void PredictionObjects::Update(const NumaEngine::Vector3& start, const NumaEngine::Vector3& goal)
{
	// データを取得する
	const auto& data = items_->GetEffectData();
	float scale = 1.0f / static_cast<float>(predictionObjects_.size() + 1);
	NumaEngine::Vector3 velocity = (goal - start) * scale;

	for (size_t i = 0; i < predictionObjects_.size(); ++i) {
		NumaEngine::Vector3 position = velocity * static_cast<float>(i + 1) + start;
		NumaEngine::Vector3 prePosition = predictionObjects_[i]->GetTransform().translation_;
		position.y = 1.0f;
		prePosition.y = 1.0f;
		predictionObjects_[i]->SetTransformRotation(NumaEngine::Quaternion::DirectionToQuaternion(NumaEngine::Quaternion::IdentityQuaternion(), velocity, 1.0f));
		predictionObjects_[i]->SetTransformTranslation(NumaEngine::Vector3::Lerp(prePosition, position, data.lerpSpeed));
	}
}
