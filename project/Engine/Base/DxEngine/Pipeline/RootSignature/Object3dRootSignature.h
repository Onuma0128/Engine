...
public:
	...
	ComPtr<ID3D12RootSignature> BuildRootSignature(ID3D12Device* device, PostEffectType effectType = PostEffectType::kNone) override
	{
		...
		rootParameters[9].DescriptorTable.pDescriptorRanges = shadowMapRnage;				//Tableの中身の配列を指定
		rootParameters[9].DescriptorTable.NumDescriptorRanges = _countof(shadowMapRnage);	//Tableで利用する数
		...
		staticSamplers[1].AddressU = D3D12_TEXTURE_ADDRESS_MODE_CLAMP; //0~1の範囲をクランプ
